            ...
            if (board.data && typeof board.data === 'string') {
                try {
                    ...
                    if (boardData.contacts && Array.isArray(boardData.contacts)) {
                        console.log(`- Processing board "${board.name}" (ID: ${board.id}). Found ${boardData.contacts.length} contacts.`);
                        ...
                        for (const contact of boardData.contacts) {
                            try {
                                ...
                                if (!contactData.email) {
                                    console.log(`  - Skipping contact "${contact.name}" due to missing email.`);
                                    continue;
                                }
                                ...
                                if (err.code === 'SQLITE_CONSTRAINT') {
                                    console.log(`  - Contact already exists (skipping): ${contact.name} (${contact.email})`);
                                } else {
                                    console.error(`  - Error migrating contact "${contact.name}":`, err.message);
                                }
                            }
                        }
                    }
                } catch (e) {
                    console.error(`- Error parsing data for board ID ${board.id}, skipping.`, e.message);
                }
            }
        }
