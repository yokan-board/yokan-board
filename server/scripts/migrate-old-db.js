        ...
        for (const board of boards) {
            ...
            if (boardData.contacts && Array.isArray(boardData.contacts)) {
                for (const contact of boardData.contacts) {
                    if (!contact.email) {
                        console.warn(
                            `Contact ${contact.name} in board ${board.id} has no email, skipping.`
                        );
                        continue;
                    }
                    ...
                    if (userContacts.has(contact.email)) {
                        ...
                        try {
                            ...
                            if (err.message.includes('UNIQUE constraint failed')) {
                                ...
                                console.error(
                                    `Error migrating contact ${contact.email}:`,
                                    err.message
                                );
                            }
                        }
            ...
            const placeholders = Object.keys(board)
                .map(() => '?')
                .join(', ');
